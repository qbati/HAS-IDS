...
import matplotlib
matplotlib.use("Agg")  # backend for saving figures without display
...
def main():
    ...
    # --- Load data ---
    X_train, y_train, X_test, y_test, df_test_orig, scaler, feature_cols, num_cols, cat_cols = \
        load_and_preprocess_unsw_nb15(TRAIN_PATH, TEST_PATH)
    ...
    try:
        # NOTE: this follows your existing code; it uses (1 - quantile) as before
    ...
    # ----------------------------------------------------------------------
    print("\nComputing Module-1 embeddings for UNSW-NB15 test set (for t-SNE)...")
    model.eval()
    with torch.no_grad():
        Z_test_list = []
    ...
    acc, prec, rec, f1, fpr = score_detail(
        y_test, y_pred, y_score, title="Test Set Results (HAS-IDS)"
    )

    # ----------------------------------------------------------------------
    # t-SNE visualisations
    # ----------------------------------------------------------------------
    print("\nRunning t-SNE on UNSW-NB15 test set (this may take a bit)...")

    max_points = 15000
    n_test = X_test.shape[0]
    if n_test > max_points:
        sss_vis = StratifiedShuffleSplit(n_splits=1, test_size=max_points, random_state=SEED)
    ...
    plt.tight_layout()
    plt.savefig("unsw_tsne_original_vs_repr.png", dpi=300, bbox_inches="tight")
    plt.savefig("unsw_tsne_original_vs_repr.pdf", dpi=300, bbox_inches="tight")
    plt.close()
    print("Saved UNSW-NB15 t-SNE figure to unsw_tsne_original_vs_repr.[png|pdf]")

    # ----------------------------------------------------------------------
    # Histogram of fused attack scores p_attack
    # ----------------------------------------------------------------------
    print("\nPlotting histogram of fused attack scores p_attack on UNSW-NB15...")
    ...
    plt.tight_layout()
    plt.savefig("unsw_hist_pattack.png", dpi=300, bbox_inches="tight")
    plt.savefig("unsw_hist_pattack.pdf", dpi=300, bbox_inches="tight")
    plt.close()
    print("Saved UNSW-NB15 histogram figure to unsw_hist_pattack.[png|pdf]")


    # ----------------------------------------------------------------------
    # Save predictions / metrics / artifacts
    # ----------------------------------------------------------------------
