...
# (Optional) cap CPU threading to keep sklearn / BLAS predictable
...
# ----------------------------------------------------------------------------
# 4. HAS-IDS DECISION FRAMEWORK HELPERS (BGMM + context, NO ANN)
# ----------------------------------------------------------------------------
...
# ----------------------------------------------------------------------------
# 5. MAIN EXECUTION BLOCK (w/o Instance-Based Score)
# ----------------------------------------------------------------------------
...
def main():
    ...
    # BGMM candidates for calibration
    BGMM_CANDIDATES = [(3, 4e-3), (4, 8e-3), (5, 1e-2)]

    HCN_GUARD_QUANTILE = 0.995  # same as before
    ...
    print("\nSTARTING DEEP FEATURE EXTRACTOR TRAINING (w/o Instance-Based Score)")
    ...
    # --- Calibration Phase ---
    print("\nSTARTING CALIBRATION PHASE (w/o Instance-Based Score)")
    ...
    Z_cal_raw = Z_train_np[cal_idx]  # already L2-normalized
    ...
    # --- Meta-Classifier Feature Engineering on Calibration Set (NO ANN) ---
    ...
    print("\nTRAINING ROBUST STACKED META-CLASSIFIER (NO Instance-Based Score)")
    meta_pipe = Pipeline(
        [
            ("std", StandardScaler()),
    ...
    # ----------------------------------------------------------------------
    # Stop training timer here (encoder + BGMM + meta-classifier)
    # ----------------------------------------------------------------------
    ...
    # --- Final Inference ---
    print("\nPERFORMING FINAL INFERENCE ON TEST SET (w/o Instance-Based Score)")
    ...
    acc, prec, rec, f1, fpr = score_detail(
        ...
        title="Test Set Results (HAS-IDS w/o Instance-Based Score)"
    )
    ...
    pred_path = "unsw_nb15_hasids_no_instance_test_predictions.csv"
    ...
    with open("unsw_nb15_hasids_no_instance_test_metrics.json", "w") as f:
        json.dump(metrics, f, indent=2)
    print("Metrics + computational cost saved to unsw_nb15_hasids_no_instance_test_metrics.json")

    # --- Save Artifacts ---
    print("\nSaving final HAS-IDS (w/o Instance-Based Score) model artifacts...")
    try:
        artifacts = {
            ...
            "ann_config": None,       # explicitly None in this ablation
            "attack_bank": None,      # no attack bank
            "feature_columns": feature_cols,
            "num_cols": num_cols,
            "cat_cols": cat_cols,
            "model_version": "hasids_unsw_nb15_no_instance",
        }
        joblib.dump(artifacts, "hasids_unsw_nb15_no_instance_artifacts.joblib")
        print("Artifacts saved to hasids_unsw_nb15_no_instance_artifacts.joblib")
    except Exception as e:
