...
# ----------------------------------------------------------------------------
# 4. HAS-IDS DECISION FRAMEWORK HELPERS (BGMM + ANN)
# ----------------------------------------------------------------------------
...
# ----------------------------------------------------------------------------
# 5. MAIN EXECUTION BLOCK — I + C (no P)
# ----------------------------------------------------------------------------
...
def main():
    ...
    HCN_GUARD_QUANTILE = 0.995  # same style as your full UNSW pipeline
    ...
    # ----------------------------------------------------------------------
    # TRAINING TIMER (encoder + BGMM + ANN + meta on I+C)
    # ----------------------------------------------------------------------
    ...
    print("\nSTARTING DEEP FEATURE EXTRACTOR TRAINING (I + C, no P)")
    ...
    # --- Embed full train & build P(z) internally (for BGMM only) ---
    model.eval()
    with torch.no_grad():
        Z_list, y_list = [], []
    ...
    # --- Calibration split ---
    print("\nSTARTING CALIBRATION PHASE (I + C, no P)")
    ...
    Z_cal_raw = Z_train_np[cal_idx]  # for ANN

    # --- Build ATTACK BANK for ANN ---
    ...
    # --- BGMM hyperparameter search based on P(z) (still internal) ---
    ...
    for (NC, RC) in BGMM_CANDIDATES:
        ...
        # context-only check (temporary meta on I+C) for selection
        pN_cal_tmp, resp_cal_tmp = posterior_normal(bgmm_temp, z_tr_cal, comp_label_temp)
        ann_scores_cal_tmp = ann_attack_score(Z_cal_raw, faiss_index, k=ANN_K, fallback_bank=attack_bank)
        comp_means_tmp = bgmm_temp.means_[resp_cal_tmp.argmax(axis=1)].flatten()
        comp_vars_tmp  = bgmm_temp.covariances_[resp_cal_tmp.argmax(axis=1)].flatten()
    ...
    # --- Final meta-features (I + C only) on calibration ---
    ...
    # *** I + C ONLY: drop pN from meta features ***
    ...
    print("\nTRAINING META-CLASSIFIER (I + C only, UNSW)")
    meta_pipe = Pipeline([
        ("std", StandardScaler()),
        ("lr", LogisticRegression(C=1.0, class_weight="balanced",
                                  solver="liblinear", random_state=SEED))
    ...
    print(f"[Meta (I+C)] Optimal decision threshold = {optimal_threshold:.4f}")

    try:
        # UNSW full code style: use 1 - quantile
    ...
    # ----------------------------------------------------------------------
    print("\nPERFORMING FINAL INFERENCE ON TEST SET (I + C, no P)")
    ...
    acc, prec, rec, f1, fpr = score_detail(
        ...
        title="Test Set Results (HAS-IDS Ablation: I + C (no P), UNSW-NB15)"
    )
    ...
    pred_df["meta_ic_score"]  = y_score   # fusion (I+C) score

    pred_path = "unsw_nb15_hasids_IplusC_noP_test_predictions.csv"
    ...
    with open("unsw_nb15_hasids_IplusC_noP_test_metrics.json", "w") as f:
        json.dump(metrics, f, indent=2)
    print("Metrics + computational cost saved to unsw_nb15_hasids_IplusC_noP_test_metrics.json")

    # --- Save Artifacts ---
    print("\nSaving HAS-IDS (I + C, no P, UNSW-NB15) artifacts...")
    try:
        artifacts = {
            ...
            "num_cols": num_cols,
            "cat_cols": cat_cols,
            "model_version": "hasids_unsw_nb15_IplusC_noP",
        }
        joblib.dump(artifacts, "hasids_unsw_nb15_IplusC_noP_artifacts.joblib")
        print("Artifacts saved to hasids_unsw_nb15_IplusC_noP_artifacts.joblib")
    except Exception as e:
