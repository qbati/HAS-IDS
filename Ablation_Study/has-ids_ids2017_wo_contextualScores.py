...
def find_optimal_meta_threshold(y_true, y_scores, target_fpr=0.06):
    ...
    for thresh in thresholds:
        ...
        if fpr <= target_fpr:
            ...
            candidates.append({'thresh': thresh, 'f1': f1, 'fpr': fpr})
...
# ----------------------------------------------------------------------------
# 5. MAIN EXECUTION BLOCK (w/o Contextual Score: Prob + ANN)
# ----------------------------------------------------------------------------
...
def main():
    ...
    with torch.no_grad():
        ...
        if len(z_norm_samples) >= 10:
            cdist = 1 - F.cosine_similarity(z_norm_samples, z_norm_samples.mean(dim=0, keepdim=True))
    ...
    # --- Meta-Classifier Feature Engineering on Calibration Set (Prob + ANN only) ---
    ...
    # w/o Contextual Score: meta features = [pN, ANN]
    ...
    print("\nTRAINING ROBUST STACKED META-CLASSIFIER (Prob + ANN, no Context)")
    meta_pipe = Pipeline([
        ('std', StandardScaler()),
    ...
    # --- Final Inference ---
    print("\nPERFORMING FINAL INFERENCE ON TEST SET (w/o Contextual Score)")
    ...
    acc, prec, rec, f1, fpr = score_detail(
        ...
        title="Test Set Results (HAS-IDS w/o Contextual Score: Prob + ANN, CIC-IDS-2017)"
    )
    ...
    pred_path = "cicids2017_hasids_wo_context_0.001_test_predictions.csv"
    ...
    with open("cicids2017_hasids_wo_context_test_metrics.json", "w") as f:
        json.dump(metrics, f, indent=2)
    print("Metrics + computational cost saved to cicids2017_hasids_wo_context_test_metrics.json")

    # --- Save Artifacts ---
    print("\nSaving HAS-IDS (w/o Contextual Score, Prob + ANN) artifacts for CIC-IDS-2017...")
    try:
        artifacts = {
            ...
            'num_cols': num_cols,
            'cat_cols': cat_cols,
            'model_version': 'hasids_wo_context_faiss_ann_cicids2017_v1'
        }
        joblib.dump(artifacts, "cicids2017_hasids_wo_context_faiss_ann_artifacts.joblib")
        print("Artifacts saved to cicids2017_hasids_wo_context_faiss_ann_artifacts.joblib")
