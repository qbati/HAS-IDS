...
def build_faiss_index(
    ...
    if method == "flatip":
        ...
        nlist_eff = max(1, min(nlist, bank_vecs.shape[0]))
...
# ----------------------------------------------------------------------------
# 5. MAIN EXECUTION BLOCK (w/o Contextual Score)
# ----------------------------------------------------------------------------
...
def main():
    ...
    print("\nSTARTING DEEP FEATURE EXTRACTOR TRAINING (w/o Contextual Score)")
    ...
    # --- Calibration Phase ---
    print("\nSTARTING CALIBRATION PHASE (w/o Contextual Score)")
    ...
    Z_cal_raw = Z_train_np[cal_idx]  # NOTE: already L2-normalized
    ...
    # --- Meta-Classifier Feature Engineering on Calibration Set (NO CONTEXT) ---
    ...
    # NO contextual features here (no component_means, no component_vars)
    ...
    # --- Meta-Classifier Training (LR on [pN, ANN]) ---
    print("\nTRAINING ROBUST STACKED META-CLASSIFIER (w/o Contextual Score)")
    meta_pipe = Pipeline(
        [
            ("std", StandardScaler()),
            (
    ...
    for xb, _ in DataLoader(test_ds, batch_size=BATCH, shuffle=False):
        ...
        # Build meta-features *without* contextual stats
    ...
    acc, prec, rec, f1, fpr = score_detail(
        ...
        title="Test Set Results (HAS-IDS w/o Contextual Score)"
    )
    ...
    pred_path = "unsw_nb15_hasids_no_context_test_predictions.csv"
    ...
    with open("unsw_nb15_hasids_no_context_test_metrics.json", "w") as f:
        json.dump(metrics, f, indent=2)
    print("Metrics + computational cost saved to unsw_nb15_hasids_no_context_test_metrics.json")

    # --- Save Artifacts ---
    print("\nSaving final HAS-IDS (w/o Contextual Score) model artifacts...")
    try:
        artifacts = {
            ...
            "num_cols": num_cols,
            "cat_cols": cat_cols,
            "model_version": "hasids_unsw_nb15_no_context",
        }
        joblib.dump(artifacts, "hasids_unsw_nb15_no_context_artifacts.joblib")
        print("Artifacts saved to hasids_unsw_nb15_no_context_artifacts.joblib")
    except Exception as e:
