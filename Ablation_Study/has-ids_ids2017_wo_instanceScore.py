...
PREFERRED_FAISS_DEVICE  = "cpu"    # "auto" | "gpu"  | "cpu"  # unused in this ablation
# ============================================================================
...
# FAISS is imported but NOT used in this ablation (no instance-based score).
...
# ----------------------------------------------------------------------------
# 4. HAS-IDS DECISION FRAMEWORK HELPERS (Prob + Context only)
# ----------------------------------------------------------------------------
...
def find_optimal_meta_threshold(y_true, y_scores, target_fpr=0.06):
    ...
    return float(best_candidate['thresh'])


# ----------------------------------------------------------------------------
# 5. MAIN EXECUTION BLOCK (HAS-IDS w/o Instance-Based Score: Prob + Context)
# ----------------------------------------------------------------------------
...
def main():
    ...
    COVER_GAMMA = 0.97
    TARGET_FPR  = 0.10  # Deployment target FPR
    ...
    print(f"[BGMM] Selected n_components={best_bgmm_config['nc']}, reg_covar={best_bgmm_config['rc']:.1e} via calibration.")

    # --- Meta-Classifier Feature Engineering on Calibration Set (Prob + Context only) ---
    pN_cal_raw, resp_cal = posterior_normal(bgmm, z_tr_cal, comp_label)

    # Contextual features from BGMM: component-wise mean and variance
    comp_idx_cal = resp_cal.argmax(axis=1)
    component_means = bgmm.means_[comp_idx_cal].flatten()
    component_vars  = bgmm.covariances_[comp_idx_cal].flatten()

    # HAS-IDS w/o Instance-Based Score: meta features = [pN, mean, var]
    ...
    print("\nTRAINING ROBUST STACKED META-CLASSIFIER (Prob + Context, no ANN)")
    meta_pipe = Pipeline([
        ('std', StandardScaler()),
    ...
    # ----------------------------------------------------------------------
    # Stop training timer here (encoder + BGMM + meta-classifier, no ANN)
    # ----------------------------------------------------------------------
    ...
    # --- Final Inference ---
    print("\nPERFORMING FINAL INFERENCE ON TEST SET (HAS-IDS w/o Instance-Based Score)")
    ...
    for xb, _ in DataLoader(test_ds, batch_size=512, shuffle=False):
        with torch.no_grad():
            ...
            z_batch_scores = (0.5 * torch.log((1 + s_batch) / (1 - s_batch + 1e-9))).cpu().numpy()

        pN_batch_raw, resp_batch = posterior_normal(bgmm, z_batch_scores, comp_label)

        # Context on test: component mean / var
        comp_idx_batch = resp_batch.argmax(axis=1)
        component_means_batch = bgmm.means_[comp_idx_batch].flatten()
        component_vars_batch  = bgmm.covariances_[comp_idx_batch].flatten()
    ...
    acc, prec, rec, f1, fpr = score_detail(
        ...
        title="Test Set Results (HAS-IDS w/o Instance-Based Score: Prob + Context, CIC-IDS-2017)"
    )
    ...
    pred_path = "cicids2017_hasids_wo_instance_0.001_test_predictions.csv"
    ...
    with open("cicids2017_hasids_wo_instance_test_metrics.json", "w") as f:
        json.dump(metrics, f, indent=2)
    print("Metrics + computational cost saved to cicids2017_hasids_wo_instance_test_metrics.json")

    # --- Save Artifacts ---
    print("\nSaving HAS-IDS (w/o Instance-Based Score, Prob + Context) artifacts for CIC-IDS-2017...")
    try:
        artifacts = {
            ...
            # No attack_bank / FAISS used in this ablation
            'attack_bank': None,
            'feature_columns': feature_columns,
            'num_cols': num_cols,
            'cat_cols': cat_cols,
            'model_version': 'hasids_wo_instance_prob_context_cicids2017_v1'
        }
        joblib.dump(artifacts, "cicids2017_hasids_wo_instance_artifacts.joblib")
        print("Artifacts saved to cicids2017_hasids_wo_instance_artifacts.joblib")
