...
# ----------------------------------------------------------------------------
# 5. MAIN EXECUTION BLOCK — CONTEXT ONLY
# ----------------------------------------------------------------------------
...
def main():
    ...
    HCN_GUARD_QUANTILE = 0.995  # used to carve out very-high-confidence normals
    ...
    # ----------------------------------------------------------------------
    # TRAINING TIMER (encoder + BGMM context)
    # ----------------------------------------------------------------------
    ...
    print("\nSTARTING DEEP FEATURE EXTRACTOR TRAINING (Context-only)")
    ...
    # --- Embed full train & build 1-D P(z) channel (internal only) ---
    model.eval()
    with torch.no_grad():
        Z_list, y_list = [], []
    ...
    # --- Calibration split ---
    print("\nSTARTING CALIBRATION PHASE (BGMM context)")
    ...
    # --- BGMM hyperparameter search based on context score C = 1 - pN ---
    best_cfg = {"f1": -1.0}
    for (NC, RC) in BGMM_CANDIDATES:
        ...
        c_cal_tmp = 1.0 - pN_cal_tmp   # context anomaly score
    ...
    # recompute pN & context C on cal with best BGMM, then refine threshold & HCN guard
    ...
    print(f"[Context-only] Optimal decision threshold (C) = {optimal_threshold:.6f}")

    try:
        # very-low C on normals → high-confidence-normal guard
    ...
    # ----------------------------------------------------------------------
    print("\nPERFORMING FINAL INFERENCE ON TEST SET (Context-only)")
    ...
    with torch.no_grad():
        for xb, _ in DataLoader(test_ds, batch_size=BATCH, shuffle=False):
            ...
            c_batch = 1.0 - pN_batch  # context anomaly score
    ...
    acc, prec, rec, f1, fpr = score_detail(
        ...
        title="Test Set Results (HAS-IDS Ablation: Context-only, UNSW-NB15)"
    )
    ...
    pred_path = "unsw_nb15_hasids_context_only_test_predictions.csv"
    ...
    with open("unsw_nb15_hasids_context_only_test_metrics.json", "w") as f:
        json.dump(metrics, f, indent=2)
    print("Metrics saved to unsw_nb15_hasids_context_only_test_metrics.json")

    # --- Save artifacts ---
    print("\nSaving HAS-IDS (Context-only, UNSW-NB15) artifacts...")
    try:
        artifacts = {
            ...
            "num_cols": num_cols,
            "cat_cols": cat_cols,
            "model_version": "hasids_unsw_nb15_context_only",
        }
        joblib.dump(artifacts, "hasids_unsw_nb15_context_only_artifacts.joblib")
        print("Artifacts saved to hasids_unsw_nb15_context_only_artifacts.joblib")
    except Exception as e:
