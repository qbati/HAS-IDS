...
# --- Encoder: 196 -> 128 -> 128 -> 64 ---
class EncoderUNSW(nn.Module):
...
# --- Asymmetric contrastive loss (Eqs. 2–5) ---
...
def main():
    ...
    # standardise label column and drop IDs
    ...
    y_tr = df_tr["binary_label"].astype(int).values
    y_te = df_te["binary_label"].astype(int).values

    feat_cols = [c for c in df_tr.columns if c not in ("binary_label", "attack_cat")]

    # 2) Global preprocessing (leaky on purpose)
    print(".. one-hot + MinMax scaling (train + test) ..")
    df_all = pd.concat([df_tr[feat_cols], df_te[feat_cols]], ignore_index=True)
    df_all = pd.get_dummies(df_all).replace([np.inf, -np.inf], np.nan).fillna(0.0)
    ...
    if X_tr.shape[1] != 196:
        raise ValueError(f"Expected 196-dim features, got {X_tr.shape[1]}")

    # 3) Train encoder
    model = EncoderUNSW(X_tr.shape[1]).to(DEVICE)
    ...
    for ep in range(EPOCHS):
        ...
        if (ep + 1) in {10, 20, 50, 100, 150, EPOCHS}:
    ...
    ).to_csv("UNSW_preds.csv", index=False)
    joblib.dump(
